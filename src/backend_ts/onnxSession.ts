...
function getCandidateModelPaths(): string[] {
  const paths: string[] = [];
  if (process.env.STEG_ONNX_PATH && process.env.STEG_ONNX_PATH.trim().length > 0) {
    paths.push(path.resolve(process.env.STEG_ONNX_PATH.trim()));
  }
  paths.push(
    ...
    path.join(process.cwd(), 'models/cost_map_lfrinn.onnx')
  );
  return paths;
}
...
export async function initOnnxSession(): Promise<ort.InferenceSession | null> {
  ...
  console.log(`[ONNX Trace] Startup process.cwd(): ${cwd}`);

  const candidatePaths = getCandidateModelPaths();
  console.log(`[ONNX Trace] Evaluating candidate model paths:`);
  ...
  try {
    ...
    if (!modelPath) {
      const errMsg = `ONNX model file not found in searched paths: ${candidatePaths.join(', ')}`;
      console.error(`[ONNX Trace] Critical: ${errMsg}`);
    ...
    console.log(`[ONNX Trace] Attempting ort.InferenceSession.create on: ${modelPath}`);
    ...
    initError = err;
    console.error(`[ONNX] Detailed Initialization Error:`, err);
 ...
 */
export function getOnnxInitError(): Error | null {
  return initError;
}
